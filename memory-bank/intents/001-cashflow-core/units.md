...
Das Intent zerlegt sich in 8 Units: 7 Backend-Services (DDD) + 1 Frontend-Unit.
...
**Description**: Buchungszeilen-Abfrage mit Filtern, Bulk-Operationen und aggregierte Jahres-/Monatsauswertung fuer Einnahmen, Ausgaben und Erfolgsneutrale Zahlungen.

**Assigned FRs**: FR-9, FR-10, FR-11, FR-23

**Deliverables**:
- Journal-Lines-Query-API (Filter: Konto, Partner, Datum, Status)
- Bulk-Assign-Partner-API
- Audit-Log-API (für Admin/Mandant-Admin)
- Income-Expense-Matrix-API (Jahresspalte + Monate, inkl. Gruppen- und Gesamtsummen fuer income/expense/neutral)
...
**Assigned FRs**: Alle user-facing FRs (inkl. FR-12 bis FR-23)
...
- Einnahmen-&-Ausgaben-Screen mit Jahresnavigation, Gruppen, Drag & Drop und Summenlogik
...
**Description**: Leistungs-Stammdatenverwaltung, Service Matcher, automatische und manuelle Leistungszuordnung, Revalidierung, Service-Typ-Ermittlung, Keyword-Konfiguration, Service-Gruppierung fuer Reporting und Trigger für Review-Item-Erstellung.

**Assigned FRs**: FR-12, FR-13, FR-14, FR-15, FR-16, FR-17, FR-21, FR-23

**Deliverables**:
- Service-CRUD-API (inkl. Basisleistung-Schutz)
- Service-Matcher-API (inkl. Regex-Validierung)
- Service-Group-API (CRUD fuer Gruppen + Zuordnung Leistung→Gruppe)
