    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <div className="mb-4">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
          Bereits bestätigte, korrigierte oder abgelehnte Prüfentscheidungen mit Filtern für Typ,
          Bearbeiter und Zeitraum.
        </p>
      </div>
      ...
      <section className="mb-6 grid gap-3 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-4">
        ...
        <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">
          ...
          <input
            ...
            onChange={(event) => setResolvedByUserId(event.target.value)}
            placeholder="UUID"
            className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm text-slate-900"
          />
        </label>
        <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">
          ...
          <input
            type="date"
            value={resolvedFrom}
            onChange={(event) => setResolvedFrom(event.target.value)}
            className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm text-slate-900"
          />
        </label>
        <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">
          ...
          <input
            type="date"
            value={resolvedTo}
            onChange={(event) => setResolvedTo(event.target.value)}
            className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm text-slate-900"
          />
        </label>
      </section>
      ...
      {isLoading ? (
        ...
        <EmptyReviewState
          title="Kein Archivtreffer"
          text="Für die gewählten Filter wurden noch keine abgeschlossenen Review-Entscheidungen gefunden."
        />
      ) : (
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-3 text-sm text-slate-500">
                    {formatReviewReason(item.context.reason)}
                  </p>
                </div>
                <div className="text-right text-xs text-slate-400">
                  <div>
                    Erledigt:{' '}
                    {item.resolved_at ? new Date(item.resolved_at).toLocaleString('de-DE') : '—'}
                  </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {item.journal_line ? (
                  <div className="rounded-2xl bg-slate-50 p-4">
                    ...
                    <p className="mt-2 text-sm font-semibold text-slate-900">
                      {item.journal_line.partner_name_raw ?? 'Ohne Partnername'}
                    </p>
                    <p className="mt-1 text-sm text-slate-500">
                      {item.journal_line.text ?? 'Kein Buchungstext vorhanden'}
                    </p>
                ...
                {item.service ? (
                  <div className="rounded-2xl bg-slate-50 p-4">
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Typ: {serviceTypeLabels[item.service.service_type]}
                    </p>
                    <p className="mt-1 text-sm text-slate-500">
                      Steuersatz: {item.service.tax_rate}%
                    </p>
                  </div>
                ) : null}
              </div>
