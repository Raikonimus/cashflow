...
class MandantService:
    ...
    async def update_mandant(
        self, mandant_id: UUID, data: UpdateMandantRequest
    ) -> Mandant:
    ...
    async def deactivate_mandant(self, mandant_id: UUID) -> None:
        ...
        result = await self._session.exec(
            select(Account).where(
                Account.mandant_id == mandant_id, Account.is_active == True
            )  # noqa: E712
        )
        accounts = result.all()
        ...
        log.info(
            ...
            accounts_deactivated=len(accounts),
        )
    ...
    async def _collect_inventory(self, mandant_id: UUID) -> dict[str, int]:
        account_ids = await self._select_ids(
            Account.id, Account.mandant_id == mandant_id
        )
        partner_ids = await self._select_ids(
            Partner.id, Partner.mandant_id == mandant_id
        )
        service_ids = await self._select_ids_for_values(
            Service.id, Service.partner_id, partner_ids
        )
        journal_line_ids_all = await self._select_ids_for_values(
            JournalLine.id, JournalLine.account_id, account_ids
        )
        journal_line_ids_partner = await self._select_ids_for_values(
            JournalLine.id, JournalLine.partner_id, partner_ids
        )
        review_ids_journal = await self._select_ids_for_values(
            ReviewItem.id, ReviewItem.journal_line_id, journal_line_ids_all
        )
        review_ids_partner = await self._select_ids_for_values(
            ReviewItem.id, ReviewItem.journal_line_id, journal_line_ids_partner
        )
        review_ids_service = await self._select_ids_for_values(
            ReviewItem.id, ReviewItem.service_id, service_ids
        )
        ...
        return {
            ...
            "mandant_users": len(
                await self._select_ids(
                    MandantUser.user_id, MandantUser.mandant_id == mandant_id
                )
            ),
            "accounts": len(account_ids),
            "column_mapping_configs": len(
                await self._select_ids_for_values(
                    ColumnMappingConfig.id, ColumnMappingConfig.account_id, account_ids
                )
            ),
            "account_excluded_identifiers": len(
                await self._select_ids_for_values(
                    AccountExcludedIdentifier.id,
                    AccountExcludedIdentifier.account_id,
                    account_ids,
                )
            ),
            "import_runs": len(
                await self._select_ids(ImportRun.id, ImportRun.mandant_id == mandant_id)
            ),
            "journal_lines_all": len(journal_line_ids_all),
            "journal_lines_partner": len(journal_line_ids_partner),
            "review_items_all": len(
                await self._select_ids(
                    ReviewItem.id, ReviewItem.mandant_id == mandant_id
                )
            ),
            "review_items_journal": len(review_ids_journal),
            "review_items_partner": len(
                set(review_ids_partner) | set(review_ids_service)
            ),
            "partners": len(partner_ids),
            "partner_ibans": len(
                await self._select_ids_for_values(
                    PartnerIban.id, PartnerIban.partner_id, partner_ids
                )
            ),
            "partner_accounts": len(
                await self._select_ids_for_values(
                    PartnerAccount.id, PartnerAccount.partner_id, partner_ids
                )
            ),
            "partner_names": len(
                await self._select_ids_for_values(
                    PartnerName.id, PartnerName.partner_id, partner_ids
                )
            ),
            "services": len(service_ids),
            "service_matchers": len(
                await self._select_ids_for_values(
                    ServiceMatcher.id, ServiceMatcher.service_id, service_ids
                )
            ),
            "service_type_keywords": len(
                await self._select_ids(
                    ServiceTypeKeyword.id, ServiceTypeKeyword.mandant_id == mandant_id
                )
            ),
            "audit_log": len(
                await self._select_ids(AuditLog.id, AuditLog.mandant_id == mandant_id)
            ),
        }
    ...
    async def _delete_all_data(self, mandant_id: UUID) -> list[CleanupPreviewItem]:
        ...
        deleted_items.extend(
            await self._delete_journal_scope(mandant_id, include_related_reviews=False)
        )
        deleted_items.extend(
            await self._delete_partner_scope(mandant_id, include_related_reviews=False)
        )
        ...
        deleted_items.append(
            await self._delete_for_values(
                ColumnMappingConfig,
                ColumnMappingConfig.account_id,
                account_ids,
        ...
        deleted_items.append(
            await self._delete_for_values(
                ...
                "Ausgeschlossene Konto-Identifikatoren",
            )
        )
        deleted_items.append(
            await self._delete_where(
                ...
                "Mandanten-Keyword-Regeln",
            )
        )
        deleted_items.append(
            await self._delete_where(
                Account, Account.mandant_id == mandant_id, "accounts", "Konten"
            )
        )
    ...
    async def _delete_mandant_record(
        self, mandant_id: UUID
        ...
        deleted_items = [
            ...
            await self._delete_where(
                Mandant, Mandant.id == mandant_id, "mandant", "Mandant"
            ),
        ]
        await self._session.commit()
        return [item for item in deleted_items if item.count > 0]

    async def _delete_journal_scope(
        self, mandant_id: UUID, include_related_reviews: bool = True
    ) -> list[CleanupPreviewItem]:
        account_ids = await self._select_ids(
            Account.id, Account.mandant_id == mandant_id
        )
        journal_line_ids = await self._select_ids_for_values(
            JournalLine.id, JournalLine.account_id, account_ids
        )
        touched_service_ids = await self._select_ids_for_values(
            JournalLineSplit.service_id,
            JournalLineSplit.journal_line_id,
            journal_line_ids,
            skip_none=True,
        )
        deleted_items: list[CleanupPreviewItem] = []
        ...
        if include_related_reviews:
            deleted_items.append(
                await self._delete_for_values(
                    ReviewItem,
                    ReviewItem.journal_line_id,
                    journal_line_ids,
                    "review_items_journal",
                    "Review-Einträge zu Buchungen",
                )
            )
        deleted_items.append(
            await self._delete_for_values(
    ...
    async def _delete_partner_scope(
        self, mandant_id: UUID, include_related_reviews: bool = True
    ) -> list[CleanupPreviewItem]:
        partner_ids = await self._select_ids(
            Partner.id, Partner.mandant_id == mandant_id
        ...
        if include_related_reviews:
            deleted_items.append(
                await self._delete_for_values(
                    ...
                    ReviewItem.journal_line_id,
                    journal_line_ids,
                    "review_items_partner_journal",
                    "Review-Einträge zu Partner-Buchungen",
                )
            )
            deleted_items.append(
                await self._delete_for_values(
                    ReviewItem,
                    ReviewItem.service_id,
                    service_ids,
                    "review_items_partner_service",
                    "Review-Einträge zu Leistungen",
                )
            )
        deleted_items.append(
            await self._delete_for_values(
                ...
                JournalLine.id,
                journal_line_ids,
                "partner_journal_lines",
                "Partnerbezogene Buchungszeilen",
            )
        )
        deleted_items.append(
            await self._delete_for_values(
                ServiceMatcher,
                ServiceMatcher.service_id,
                service_ids,
                "service_matchers",
                "Leistungs-Matcher",
            )
        ...
        deleted_items.append(
            await self._delete_for_values(
                Service, Service.id, service_ids, "services", "Leistungen"
            )
        )
        deleted_items.append(
            await self._delete_for_values(
                PartnerIban,
                PartnerIban.partner_id,
                partner_ids,
                "partner_ibans",
                "Partner-IBANs",
            )
        ...
        deleted_items.append(
            await self._delete_for_values(
                Partner, Partner.id, partner_ids, "partners", "Partner"
            )
        )
    ...
    async def _select_ids_for_values(
        self, field, match_field, values: list[UUID], skip_none: bool = False
    ):
        if not values:
            return []
        result = list(
            (
                await self._session.exec(
                    select(field).where(
                        or_(*[match_field == value for value in values])
                    )
                )
            ).all()
        )
    ...
    async def _delete_where(
        self, model, condition, key: str, label: str
    ) -> CleanupPreviewItem:
    ...
    async def _delete_for_values(
        self, model, match_field, values: list[UUID], key: str, label: str
    ) -> CleanupPreviewItem:
        if not values:
    ...
    def _build_section(
        self, key: str, label: str, description: str, items: list[CleanupPreviewItem]
    ) -> CleanupPreviewSection:
        return CleanupPreviewSection(
            key=key,
            label=label,
    ...
    def _delete_mandant_items(
        self, inventory: dict[str, int]
    ) -> list[CleanupPreviewItem]:
        return [
            CleanupPreviewItem(
                key="mandant", label="Mandant", count=inventory["mandant"]
            ),
            CleanupPreviewItem(
                key="mandant_users",
                label="Mandantenzuweisungen",
                count=inventory["mandant_users"],
            ),
    ...
    def _delete_data_items(self, inventory: dict[str, int]) -> list[CleanupPreviewItem]:
        return [
            CleanupPreviewItem(
                key="accounts", label="Konten", count=inventory["accounts"]
            ),
            CleanupPreviewItem(
                key="column_mapping_configs",
                label="Spalten-Mappings",
                count=inventory["column_mapping_configs"],
            ),
            CleanupPreviewItem(
                key="account_excluded_identifiers",
                label="Ausgeschlossene Konto-Identifikatoren",
                count=inventory["account_excluded_identifiers"],
            ),
            CleanupPreviewItem(
                key="import_runs", label="Importläufe", count=inventory["import_runs"]
            ),
            CleanupPreviewItem(
                key="journal_lines_all",
                label="Buchungszeilen",
                count=inventory["journal_lines_all"],
            ),
            CleanupPreviewItem(
                key="review_items_all",
                label="Review-Einträge",
                count=inventory["review_items_all"],
            ),
            CleanupPreviewItem(
                key="partners", label="Partner", count=inventory["partners"]
            ),
            CleanupPreviewItem(
                key="partner_ibans",
                label="Partner-IBANs",
                count=inventory["partner_ibans"],
            ),
            CleanupPreviewItem(
                key="partner_accounts",
                label="Partner-Konten",
                count=inventory["partner_accounts"],
            ),
            CleanupPreviewItem(
                key="partner_names",
                label="Namensvarianten",
                count=inventory["partner_names"],
            ),
            CleanupPreviewItem(
                key="services", label="Leistungen", count=inventory["services"]
            ),
            CleanupPreviewItem(
                key="service_matchers",
                label="Leistungs-Matcher",
                count=inventory["service_matchers"],
            ),
            CleanupPreviewItem(
                key="service_type_keywords",
                label="Mandanten-Keyword-Regeln",
                count=inventory["service_type_keywords"],
            ),
            CleanupPreviewItem(
                key="audit_log",
                label="Audit-Log-Einträge",
                count=inventory["audit_log"],
            ),
        ]

    def _journal_scope_items(
        self, inventory: dict[str, int]
    ) -> list[CleanupPreviewItem]:
        return [
            CleanupPreviewItem(
            ...
            CleanupPreviewItem(
                key="review_items_journal",
                label="Review-Einträge zu Buchungen",
                count=inventory["review_items_journal"],
            ),
        ]

    def _partner_scope_items(
        self, inventory: dict[str, int]
    ) -> list[CleanupPreviewItem]:
        return [
            CleanupPreviewItem(
            ...
            CleanupPreviewItem(
                key="journal_lines_partner",
                label="Partnerbezogene Buchungszeilen",
                count=inventory["journal_lines_partner"],
            ),
            CleanupPreviewItem(
                key="review_items_partner",
                label="Review-Einträge zu Partnern und Leistungen",
                count=inventory["review_items_partner"],
            ),
        ]
...
class AccountService:
    ...
    async def create_account(
        self, mandant_id: UUID, data: CreateAccountRequest
    ) -> Account:
        if data.iban is not None:
            ...
            if existing.first() is not None:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT, detail="IBAN already in use"
                )
        else:
        ...
        log.info(
            "account_created", account_id=str(account.id), mandant_id=str(mandant_id)
        )
        return account

    async def update_account(
        self, account_id: UUID, mandant_id: UUID, data: UpdateAccountRequest
    ) -> Account:
    ...
    async def get_column_mapping(self, account_id: UUID) -> ColumnMappingConfig | None:
        result = await self._session.exec(
    ...
    async def set_column_mapping(
        self, account_id: UUID, data: ColumnMappingRequest
    ) -> ColumnMappingConfig:
    ...
    async def add_excluded_identifier(
        ...
        value = (
            ...
            if data.identifier_type == "iban"
            else data.value.strip()
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=409, detail="Identifier already excluded for this account"
            )
    ...
    async def get_excluded_sets(
        ...
        accounts = frozenset(
            e.value for e in entries if e.identifier_type == "account_number"
        )
        return ibans, accounts

    async def apply_excluded_identifiers(
        self, account_id: UUID, mandant_id: UUID
    ) -> int:
