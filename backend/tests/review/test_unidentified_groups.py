...
async def _unidentified(
    session: AsyncSession, mandant_id, text: str, amount: str, date: str = "2026-01-15"
):
    now = utcnow()
    line = JournalLine(
        id=uuid4(),
...
class TestUnidentifiedGroups:
    async def test_gruppiert_nach_haendler_und_sortiert_nach_groesse(
        ...
        await _unidentified(
            db_session,
            mandant.id,
            "ANTHROPIC inkl. Fremdwährungsentgelt 1,32 Kurs 1,14",
            "-89.00",
            "2026-01-02",
        )
        await _unidentified(
            db_session,
            mandant.id,
            "ANTHROPIC inkl. Fremdwährungsentgelt 1,18 Kurs 1,14",
            "-80.12",
            "2026-02-03",
        )
        await _unidentified(
            db_session, mandant.id, "ANTHROPIC* CLAUDE SUB", "-149.21", "2026-03-04"
        )
        await _unidentified(
            db_session, mandant.id, "MSFT * E0301094XL", "-22.50", "2026-01-20"
        )
        await db_session.commit()

        resp = await client.get(
            f"/api/v1/mandants/{mandant.id}/review/unidentified-groups",
            headers=_auth(token),
        )
    ...
    async def test_schlaegt_bestehenden_partner_statt_eines_duplikats_vor(
        ...
        await _unidentified(
            db_session,
            mandant.id,
            "WEAVIATE B.V. inkl. Fremdwährungsentgelt 0,33 Kurs 1,1271415",
            "-22.51",
        )
        await db_session.commit()
    ...
    async def test_partnername_wird_ohne_ruecksicht_auf_schreibweise_wiederverwendet(
        ...
        _, item = await _unidentified(
            db_session, mandant.id, "ANTHROPIC* CLAUDE SUB", "-99.58"
        )
        await db_session.commit()
        ...
        partners = (
            await db_session.exec(
                select(Partner).where(Partner.mandant_id == mandant.id)
            )
        ).all()
    ...
    async def test_legt_partner_leistung_matcher_an_und_ordnet_alle_zeilen_zu(
        ...
        lines = [
            (
                await _unidentified(
                    db_session,
                    mandant.id,
                    "ANTHROPIC inkl. Fremdwährungsentgelt 1,32",
                    "-89.00",
                )
            )[0],
            (
                await _unidentified(
                    db_session, mandant.id, "ANTHROPIC* CLAUDE SUB", "-149.21"
                )
            )[0],
        ]
        await db_session.commit()
        ...
        partner = await db_session.get(
            Partner, __import__("uuid").UUID(result["partner_id"])
        )
        assert partner is not None and partner.name == "Anthropic"

        service = await db_session.get(
            Service, __import__("uuid").UUID(result["service_id"])
        )
        ...
        matcher = await db_session.get(
            ServiceMatcher, __import__("uuid").UUID(result["matcher_id"])
        )
    ...
    async def test_haengt_matcher_an_bestehende_leistung_des_partners(
        ...
        line, _ = await _unidentified(
            db_session, mandant.id, "MSFT * E0301094XL", "-22.50"
        )
        await db_session.commit()
        ...
        services = (
            await db_session.exec(
                select(Service).where(Service.partner_id == partner.id)
            )
        ).all()
        assert sorted(s.name for s in services) == ["Basisleistung", "Lizenzen"]
        matchers = (
            await db_session.exec(
                select(ServiceMatcher).where(ServiceMatcher.service_id == service.id)
            )
        ).all()
    ...
    async def test_lehnt_matcher_an_der_basisleistung_ab(
        ...
        _, item = await _unidentified(
            db_session, mandant.id, "GOOGLE*ADS6139956915", "-120.00"
        )
        await db_session.commit()
    ...
    async def test_gleicher_leistungsname_legt_keine_zweite_leistung_an(
        ...
        _, first = await _unidentified(
            db_session, mandant.id, "MSFT * E0301094XL", "-22.50"
        )
        _, second = await _unidentified(
            db_session, mandant.id, "MSFT * E0300ZSOSD", "-11.00", "2026-02-15"
        )
        await db_session.commit()
        ...
        services = (
            await db_session.exec(
                select(Service).where(
                    Service.partner_id == partner.id, Service.name == "Lizenzen"
                )
            )
        ).all()
    ...
    async def test_mehrdeutige_leistung_landet_auf_der_basisleistung(
        ...
        line, _ = await _unidentified(
            db_session, mandant.id, "Mol*HF Data Datenverar...", "-14.90"
        )
        await db_session.commit()
        ...
        resp = await client.post(
            ...
            json={
                "item_ids": group["item_ids"],
                "pattern": "HF DATA DATENVERAR",
                "partner_id": str(partner.id),
                "service_name": "HF Data Datenverar",
            },
            headers=_auth(token),
        )
        assert resp.status_code == 201, resp.text
        ...
        confirmed = await client.post(
            f"/api/v1/mandants/{mandant.id}/review/{review.id}/confirm",
            headers=_auth(token),
        )
