...
async def _setup(db_session: AsyncSession, client: AsyncClient, role: UserRole = UserRole.accountant):
...
class TestAccountBalances:
    async def test_balance_is_opening_balance_plus_bookings(
        ...
        await create_journal_line_db(
            db_session, account.id, run.id, amount=Decimal("250.00"), valuta_date="2025-03-01"
        )
        await create_journal_line_db(
            db_session, account.id, run.id, amount=Decimal("-100.50"), valuta_date="2025-04-15"
        )

        resp = await client.get(BALANCES_URL.format(mandant_id=mandant.id), headers=headers)
    ...
    async def test_account_without_bookings_returns_opening_balance(
        ...
        await create_account_db(db_session, mandant.id, "Leer", opening_balance=Decimal("42.00"))
    ...
    async def test_default_opening_balance_is_zero(
        ...
        run = await create_import_run_db(db_session, account.id, mandant.id, user.id)
        await create_journal_line_db(db_session, account.id, run.id, amount=Decimal("80.00"))
    ...
    async def test_foreign_currency_lines_are_counted_not_added(
        ...
        run = await create_import_run_db(db_session, account.id, mandant.id, user.id)
        await create_journal_line_db(db_session, account.id, run.id, amount=Decimal("100.00"))
        await create_journal_line_db(
    ...
    async def test_totals_are_grouped_per_currency(
        ...
        await create_account_db(
            db_session, mandant.id, "C USD", currency="USD", opening_balance=Decimal("50.00")
        )
        run = await create_import_run_db(db_session, eur.id, mandant.id, user.id)
        await create_journal_line_db(db_session, eur.id, run.id, amount=Decimal("25.00"))
    ...
    async def test_viewer_may_read_balances(self, client: AsyncClient, db_session: AsyncSession):
