...
class TestInternalOnlyMatchers:
    async def test_interner_matcher_identifiziert_keinen_partner(
        self, db_session: AsyncSession
    ):
        mandant = await create_mandant(db_session)
        # Echter Fall: das interne Muster "Rate" steckt in "PARKGARAGE PRATERSTERN".
        await _matcher(
            db_session, mandant.id, "Forschungsförderung", "Rate", internal_only=True
        )
        ...
        result = await PartnerMatchingService(db_session).match(
            ...
            iban_raw=None,
            name_raw=None,
            text_raw="PARKGARAGE PRATERSTERN",
        )
        assert result.outcome.value == "no_partner_identified"
        assert result.partner_id is None

    async def test_oeffentlicher_matcher_identifiziert_weiterhin(
        self, db_session: AsyncSession
    ):
        mandant = await create_mandant(db_session)
        partner = await _matcher(
            db_session, mandant.id, "OpenAI", "OPENAI", internal_only=False
        )
        await db_session.flush()
    ...
    async def test_interner_matcher_erzeugt_keine_mehrdeutigkeit(
        self, db_session: AsyncSession
    ):
        mandant = await create_mandant(db_session)
        wanted = await _matcher(
            db_session, mandant.id, "OpenAI", "OPENAI", internal_only=False
        )
    ...
    async def test_partner_nur_mit_internen_matchern_bleibt_unsichtbar(
        self, db_session: AsyncSession
    ):
        mandant = await create_mandant(db_session)
        await _matcher(
            db_session, mandant.id, "Nur intern", "CAL 100", internal_only=True
        )
        await db_session.flush()
        ...
        result = await PartnerMatchingService(db_session).match(
            ...
            iban_raw=None,
            name_raw=None,
            text_raw="CAL 100-26.003",
        )
        assert result.outcome.value == "no_partner_identified"
        assert (
            result.review_context["diagnosis"]["service_matchers"]["reason"]
            == "no_matchers_configured"
        )
